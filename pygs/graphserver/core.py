...
class Graph(CShadow):
    ...
    def spt_in_place(self, fromv, tov, initstate, walk_options=None, maxtime=2000000000):
        self.check_destroyed()
        if not tov:
            tov = "*bogus^*^vertex*"
        ...
        if walk_options is None:
            walk_options = WalkOptions()
            ret = self._cspt_in_place( self.soul, fromv, tov, initstate.soul, walk_options.soul, c_long(maxtime) )
            walk_options.destroy()
            return ret
        else:
            return self._cspt_in_place( self.soul, fromv, tov, initstate.soul, walk_options.soul, c_long(maxtime) )
        
    def spt_in_place_retro(self, fromv, tov, finalstate, walk_options=None, mintime=0):
        self.check_destroyed()
        if not fromv:
            fromv = "*bogus^*^vertex*"
        ...
        if walk_options is None:
            walk_options = WalkOptions()
            ret = self._cspt_in_place_retro( self.soul, fromv, tov, finalstate.soul, walk_options.soul, c_long(mintime) )
            walk_options.destroy()
            return ret
        else:
            return self._cspt_in_place_retro( self.soul, fromv, tov, finalstate.soul, walk_options.soul, c_long(mintime) )
    
    def path_in_place(self, tov) :
        lgs.gPathInPlace( self.soul, tov );
...
class State(CShadow):
    ...
    initial_wait   = cproperty(lgs.stateGetInitialWait, c_int)
...
Graph._cspt_in_place = ccast(lgs.gSPTInPlace, Graph)
Graph._cspt_in_place_retro = ccast(lgs.gSPTInPlaceRetro, Graph)
